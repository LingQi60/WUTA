...
// Setters
    void PathGenerator::setCarState(const fsd_common_msgs::CarState &carstate) {
        car_state_ = carstate;
    }
...
// Methods
    void PathGenerator::runAlgorithm() {
        ...
        visual_trajectory(refline_, RefPath_, "/velodyne", color,
                          car_state_.header, true);
    ...
    bool PathGenerator::Check() {
        if (mission_ == "trackdrive") {
            ...
            if (local_map_.cone_red.empty() || local_map_.cone_blue.empty()) {
                ROS_INFO("local_map_.cone_red.empty()n");
        ...
        if (mission_ == "acceleration") {
            ...
            if (fabs(endPoint_.y) > 6 || endPoint_.x <= 40.0) {
                ...
                ROS_INFO("%f",endPoint_.y);
                ROS_INFO("%f",endPoint_.x);
