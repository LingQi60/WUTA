...
void LidarCluster::runAlgorithm() {
  ...
  // pcl::fromROSMsg(raw_pc2_, cloud_in);
  // std::vector<int> mapping;
  // pcl::removeNaNFromPointCloud(cloud_in,raw_pc_,mapping);
  ...
  cluster_.header.frame_id = "velodyne";
...
void LidarCluster::preprocessing(
  ...
  // Optional
  seg.setOptimizeCoefficients(true);//有毛病
  ...
  // seg.setDistanceThreshold(0.2);s
...
void LidarCluster::ClusterProcessing(
  ...
  ec.extract(cluster_indices);//有毛病
...
//   pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_cones(
//       new pcl::PointCloud<pcl::PointXYZ>);
...
// void LidarCluster::ClusterProcessing(
//     const pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud, double threshold) {
